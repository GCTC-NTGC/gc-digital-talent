...
import { AssessmentStepType, Maybe } from "@gc-digital-talent/graphql";
...
const getLabels = (intl: IntlShape, args: Args) => {
  const { type, candidateName } = args;
  switch (type) {
    default:
      return {
        ...
        subtitle: intl.formatMessage(
          {
            defaultMessage:
              "Review and assess {candidateName} evidence against the required education or equivalent experience.",
            id: "0fhUa/",
            description:
        ...
        failedOther: intl.formatMessage({
          ...
          id: "C5uhBP",
          description:
            "Label for radio option in the screening decision dialog.",
        }),
...
const useLabels = (args?: Args) => {
  const intl = useIntl();
  const labels = getLabels(intl, args ?? {}) satisfies FieldLabels;
  return labels;
};
