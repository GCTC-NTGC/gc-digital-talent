...
const EditDepartmentRoleDialog = ({
  query,
  optionsQuery,
  assignment,
}: EditDepartmentRoleDialogProps) => {
  const intl = useIntl();
  ...
  const { updateRoles, methods, fetching } = useUpdateRolesMutation<FormValues>(
    {
      userId: user.id,
      teamId: assignment.department.teamIdForRoleAssignment,
  ...
  const label = intl.formatMessage({
    ...
    id: "VgyUtZ",
    description: "Label for the form to edit a users department membership",
  });
  ...
  const roleOptions = departmentRoles.map((role) => ({
    label:
      ...
      <Dialog.Content>
        ...
        <Dialog.Body>
          ...
          <Ul>
            <li className="font-bold">
              <span>
                {assignment.department.departmentName?.localized ??
                  intl.formatMessage(commonMessages.notAvailable)}
              </span>
            </li>
          </Ul>
...
export default EditDepartmentRoleDialog;
