...
const CreatePoolDepartment_Fragment1 = graphql(/* GraphQL */ `
  fragment CreatePoolDepartment1 on Department {
    id
    departmentName: name {
      localized
    }
...
interface CreatePoolFormProps {
  ...
  classificationsQuery: FragmentType<
    typeof CreatePoolClassification_Fragment1
  >[];
  departmentsQuery: FragmentType<typeof CreatePoolDepartment_Fragment1>[];
  communitiesQuery: FragmentType<typeof CreatePoolCommunity_Fragment1>[];
  handleCreatePool: (
    userId: string,
    communityId: string,
...
export const CreatePoolForm = ({
  ...
  const classifications = getFragment(
    CreatePoolClassification_Fragment1,
    classificationsQuery,
  );
  const departments = getFragment(
    CreatePoolDepartment_Fragment1,
    departmentsQuery,
  );
  const communities = getFragment(
    CreatePoolCommunity_Fragment1,
    communitiesQuery,
  );
...
const CreatePoolPage_Query1 = graphql(/* GraphQL */ `
  query CreatePoolPage1 {
    me {
      id
      authInfo {
        roleAssignments {
          ...
          teamable {
            id
            ... on Community {
              ...CreatePoolCommunity1
            }
          }
          teamable {
            id
            ... on Department {
              ...CreatePoolDepartment1
            }
          }
        }
      }
...
const CreatePoolPage_Mutation1 = graphql(/* GraphQL */ `
  mutation CreatePool1(
    $userId: ID!
    $communityId: ID!
...
const CreatePoolPage = () => {
  ...
  const [{ data, fetching, error }] = useQuery({
    query: CreatePoolPage_Query1,
  });

  const [, executeMutation] = useMutation(CreatePoolPage_Mutation1);
