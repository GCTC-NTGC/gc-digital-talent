...
describe("Talent Search Smoke Tests", () => {
  ...
  it("searches for candidates and submits a request", () => {
    ...
    getInputByLabel('What is the job title for this position?').clear().type('Test Job Title');
    getInputByLabel('Additional Comments').clear().type('Test Comments');
    getInputByLabel('Department / Hiring Organization').select('Treasury Board Secretariat');
