    ...
    populateObject(objectToPopulate) {
        ...
        deepMapValues(objectToPopulate, (property, propertyPath) => {
            if (typeof property === 'string') {
                const populateSingleProperty = this.populateProperty(property, true).then((newProperty) => set(objectToPopulate, propertyPath, newProperty));
...
class PrismaDefinitionClass {
    ...
    async load(args, envPath, graceful) {
        if (args.project) {
            const flagPath = resolve(args.project);
            try {
                accessSync(flagPath);
            }
            catch (_a) {
...
const debug$1 = require('debug')('Environment');
class Environment {
