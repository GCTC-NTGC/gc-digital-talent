...
class PoolTest extends TestCase
{
    ...
    public function testCanAdminScope(): void
    {
        ...
        $processOperatorQuery = $this
            ->actingAs($this->processOperator, 'api')
            ->graphQL($query, [
                'where' => [
                    'canAdmin' => true,
                ],
    ...
    public function testCreatePool()
    {
        ...
        $response = $this->actingAs($this->processOperator, 'api')
            ->graphQL(
                /** @lang GraphQL */
                '
            mutation CreatePool($userId: ID!, $teamId: ID!, $communityId: ID!, $pool: CreatePoolInput!) {
                ...
                [
                    'userId' => $this->processOperator->id,
                    'teamId' => $this->team->id,
        ...
        $response->assertJsonFragment([
            'owner' => ['id' => $this->processOperator->id],
        ])->assertJsonFragment([
