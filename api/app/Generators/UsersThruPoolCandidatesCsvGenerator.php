...
class UsersThruPoolCandidatesCsvGenerator extends CsvGenerator implements FileGeneratorInterface
{
    ...
    public function generate(): self
    {
        ...
        $currentPoolCandidate = 1;
        $query = $this->buildQuery();
        $query->chunk(200, function ($poolCandidates) use ($sheet, &$currentPoolCandidate) {
            foreach ($poolCandidates as $poolCandidate) {
                ...
                $sheet->fromArray($values, null, sprintf('A%d', $currentPoolCandidate + 1));
                $currentPoolCandidate++;
            }
        });
