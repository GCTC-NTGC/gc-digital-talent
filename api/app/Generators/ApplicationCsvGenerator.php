...
class ApplicationCsvGenerator extends CsvGenerator implements FileGeneratorInterface
{
    use Filterable;
    use GeneratesFile;
    ...
    public function generate(): self
    {
        ...
        $query->chunk(200, function ($candidates) use ($sheet, &$currentCandidate) {
            foreach ($candidates as $candidate) {
                ...
                // this is the part that needs to be changed from snapshot
                ...
                $candidateSkillDetails = GeneratorUtil::getSkillDetails($candidate->pool->poolSkills, $experiences, $snapshotExperiences);

                foreach ($candidateSkillDetails as $type => $skills) {
                    foreach ($skills as $skill) {
                        $skillId = $skill['skill']['id'];
                        $this->poolSkills[$skillId][] = [
                            'candidate' => $currentCandidate,
                            'value' => implode("\r\n", array_map(function ($experience) {
                                return $experience['title'].': '.$experience['details'];
                            }, $skill['experiences']->toArray())),
                        ];
                    }
                }
